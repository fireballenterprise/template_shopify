...
1. Resolve the template repo and check whether it's a local checkout by running
   `uv run --no-sync python -m modules.template.route "pull resolve"` using the Bash tool. If it
   fails, show the full output to the user and ask how they'd like to proceed. Otherwise, parse
   `TEMPLATE_PATH=` (the resolved local path, cloned to `tmp/template_sync/` if not found locally)
   and `TEMPLATE_LOCAL=` (`true`/`false`) from the output.
